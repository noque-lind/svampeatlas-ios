...
class MushroomVC: UIViewController {
    ...
    private lazy var tableView: MushroomTable = {
        let tableView = MushroomTable()
        
        tableView.didSelectItem = { [unowned self, unowned tableView] item, indexPath in
            switch item {
            case .loadMore(offset: let offset):
                tableView.removeItem(item: item, indexPath: indexPath)
                tableView.addSection(section: .init(title: nil, state: .loading))
                ...
                DataService.instance.getMushrooms(searchString: nil, limit: limit, offset: offset, completion: { (result) in
                    ...
                    case .Success(let mushrooms):
                        ...
                        var items = mushrooms.compactMap({MushroomTable.Item.mushroom($0)})
                        
                        if mushrooms.count == limit {
                             items.append(.loadMore(offset: offset + mushrooms.count))
                        }
                        
                        tableView.addSection(section: .init(title: nil, state: .items(items: items)))
                    }
                })
    ...
    extension MushroomVC: CategoryViewDelegate {
        ...
        func categorySelected(category: Any) {
            ...
            tableView.setSections(sections: [Section(title: nil, state: .loading)])
            ...
            case .species:
                ...
                DataService.instance.getMushrooms(searchString: nil, limit: limit) { [weak self] (result) in
                    ...
                    switch result {
                    case .Error(let error):
                        self?.tableView.setSections(sections: [Section(title: nil, state: .error(error: error))])
                    ...
                    case .Success(let mushrooms):
                        ...
                        self?.tableView.setSections(sections: [Section(title: nil, state: .items(items: items))])
