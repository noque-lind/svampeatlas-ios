    ...
    @objc private func viewWasHeld(sender: UILongPressGestureRecognizer) {
        ...
        case .ended:
            ...
            if (sender.location(in: sender.view).y > (sender.view?.bounds.maxY)!) || (sender.location(in: sender.view).y < (sender.view?.bounds.minY)!) {
                ...
                case 0:
                    self.eLRevealViewController()?.pushNewViewController(viewController: UINavigationController(rootViewController: CameraVC(cameraVCUsage: CameraVC.CameraVCUsage.newObservationRecord)))
                case 1:
                    self.eLRevealViewController()?.pushNewViewController(viewController: UINavigationController(rootViewController: AddObservationVC()))
                default: return
                }
            }
        default:
