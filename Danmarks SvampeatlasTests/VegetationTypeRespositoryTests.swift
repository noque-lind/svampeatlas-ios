...
class VegetationTypeRespositoryTests: XCTestCase {
    ...
    var mockVegeationTypes: [VegetationType] = (0...5).map { VegetationType.init(id: $0, dkName: "DK NAME: \($0)", enName: "EN NAME: \($0)") }
       ...
       override func tearDown() {
        ...
        sut.vegetationTypeRepository.deleteAll { (result) in
               exp.fulfill()
           }
       ...
       func testSave() {
           ...
           sut.vegetationTypeRepository.save(items: mockVegeationTypes) { (result) in
               exp.fulfill()
           }
       ...
       func testOverride() {
        ...
        sut.vegetationTypeRepository.save(items: self.mockVegeationTypes) { (result) in
               exp.fulfill()
            self.sut.vegetationTypeRepository.save(items: self.mockVegeationTypes) { (result) in
                   exp2.fulfill()
               }
               }
